...
int AutoPath::changeColID(int id)
{
    if (id == 0)
        return 3;
    if (id == 1)
...
PathResult AutoPath::pathDecision(int col, const std::vector<std::vector<BlockType>> &grid)
{
    ...
    if (!startOnPath)
    {
        ...
        {
            take_count = 1;
            res.totalTime += g_time.forward_pick;
        }
    }
    ...
    for (uint8_t i = 0; i < 4; i++)
    {
        ...
        if (current_block == R1)
        {
            if (r1_count == 0)
            {
                res.totalTime += g_time.wait_clear_first;
                r1_count++;
            }
            else
            {
                res.totalTime += g_time.wait_clear_after;
            }
        }
        else if (current_block == R2)
        {
            ...
            {
                res.totalTime += g_time.discard_time;
            ...
            for (uint8_t k = 0; k < 2; k++)
            {
                ...
                if (sc >= 0 && sc < 3)
                {
                    if (grid[i][sc] == R2)
                    {
                        ...
                        if (!already_exists)
                        {
                            res.totalTime += g_time.side_pick;
...
PathResult AutoPath::planBestPath(const std::vector<std::vector<BlockType>> &grid)
{
    ...
    for (int i = 0; i < 3; i++)
    {
        ...
        {
            if (current.taskBlocks[0] < 4 && current.taskBlocks[0] != changeColID(current.colIndex))
            {
                current.totalTime += g_time.move_time;
            }
